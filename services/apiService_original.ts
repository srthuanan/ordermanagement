import { API_URL, MONTHS, ADMIN_USER, defaultExteriors } from '../constants';
...
const SOLD_CARS_API_URL = "https://script.google.com/macros/s/AKfycbzElHHCjX_RnObRE3VX42qlz_PnKiF8SvMKieuOwtsV1VlByJXNeZkZ2MvP9fa4ACIgIQ/exec";
...
export const cancelRequest = async (orderNumber: string, reason: string, unmatchType: string = 'Hủy luôn đơn hàng (Hủy đơn)') => {
    // 1. Ghi Supabase (Kiểm tra nếu có VIN thì trả xe về kho, Cập nhật trạng thái Đơn hàng thành Đã hủy hoặc Chờ xe)
    try {
        const currentUser = getStorageItem("currentConsultant") || "Unknown";
        const ketQuaMoi = unmatchType.includes('Chờ xe') ? 'Chưa ghép' : 'Đã hủy';

        const { data: order } = await supabase.from('donhang').select('vin').eq('so_don_hang', orderNumber).single();
        if (order && order.vin) {
            await supabase.from('khoxe').update({
            ...
            // 1.5 CẬP NHẬT ĐIỂM UY TÍN KHI HỦY ĐƠN HOẶC HỦY GHÉP
            ...
            if (matchedHold) {
                await supabase.from('car_hold_activities').update({
                    status: ketQuaMoi === 'Đã hủy' ? 'order_cancelled' : 'unmatched', // differentiate between cancel order and unmatch car
                    reason: reason,
                    updated_at: new Date().toISOString()
                 ...
                 await supabase.from('car_hold_activities').insert({
                     ...
                     tvbh_name: currentUser,
                     type: 'HOLD',
                     status: ketQuaMoi === 'Đã hủy' ? 'order_cancelled' : 'unmatched',
                     reason: reason,
                     created_at: new Date().toISOString(),
                     updated_at: new Date().toISOString()
                 });
        ...
        const updatePayload: any = {
            ket_qua: ketQuaMoi,
            vin: ketQuaMoi === 'Đã hủy' ? order?.vin : null, // If 'Chờ xe', clear the VIN completely. If 'Đã hủy' we keep the vin to indicate what was requested (or delete it, depending on business, but normally order keeps history). Actually, if we unmatched, we just clear vin anyway. Wait, Admin's unmatchOrder sets vin to null. Let's do that for both.
            thoi_gian_ghep: null
        };
        ...
        // Also update yeucauxhd if needed
        if (ketQuaMoi === 'Đã hủy') {
            await supabase.from('yeucauxhd').update({ trang_thai: 'Đã hủy', ghi_chu_admin: `Sale hủy: ${reason}` }).eq('so_don_hang', orderNumber);
        } else {
...
export const requestInvoice = async (
    ...
    await logAction('REQUEST_INVOICE', { orderNumber, policy, commission, vpoint }, orderNumber, 'order');
    ...
    try {
        if (contractFile) {
            ...
            if (existing?.url_hop_dong) {
                ...
                if (oldPath) {
                    try {
                        await supabase.storage.from('yeucauxhd-files').remove([oldPath]);
                    } catch (e) {
...
export const updateOrderDetails = async (orderNumber: string, details: Partial<Order>): Promise<ApiResult> => {
    try {
        ...
        // 3. Phản hồi cho UI TRONG TÍCH TẮC
        if (matchedVin) {
            return {
                status: 'SUCCESS',
                message: `Cập nhật thành công! Mối nối tự động ghép với VIN: ${matchedVin}`,
                autoMatched: true,
                vin: matchedVin
            };
        } else {
...
const getSoldDataForMonth = async (month: string, year: number): Promise<any[]> => {
    // Construct sheet name like "January 2026"
    ...
    try {
        ...
        // Fallback: checks if data has the exact key requested
...
const mapSoldDataRowToOrder = (row: any[], index: number, month?: string, year?: number): Order => {
    ...
    return {
        ...
        "Số đơn hàng": String(row[2] || `SOLD-${month}-${year}-${index}`), // Ensure a unique key
...
export const getSoldCarsDataByMonth = async (month: string, year: number): Promise<ApiResult> => {
    try {
        ...
        const mappedData: Order[] = rawData
            .filter(row => Array.isArray(row) && row.length > 2 && row[2]) // Filter by Order Number (index 2)
            .map((row, index) => mapSoldDataRowToOrder(row, index, month, year));
        return {
            status: 'SUCCESS',
            message: `Successfully fetched data for ${month} ${year}.`,
            data: mappedData,
        };
    } catch (error) {
        const errorMessage = error instanceof Error ? error.message : `An unknown error occurred while fetching data for ${month} ${year}.`;
...
export const getAllSoldCarsData = async (year: number): Promise<ApiResult> => {
    try {
        const fetchPromises = MONTHS.map(month =>
            getSoldDataForMonth(month, year).then(data => ({ month, data }))
        );
        ...
        const mappedData: Order[] = allSoldDataWithMonth.map(({ row, month }, index) => mapSoldDataRowToOrder(row, index, month, year));

        return {
            status: 'SUCCESS',
            message: `Successfully fetched all sold car data for ${year}.`,
            data: mappedData
        };
    } catch (error) {
...
export const getAuditLogs = async (limit: number = 100): Promise<ApiResult> => {
    try {
        ...
        // Map về định dạng AuditLog cho component TrackingDashboard
        const mappedLogs = (data || []).map(log => ({
            id: log.id,
            timestamp: log.created_at,
            user_email: log.actor_id,
            user_full_name: log.actor_name,
            action: log.type,
            details: log.metadata,
            target_id: log.target_id,
            target_type: log.target_view
        }));

        return {
            status: 'SUCCESS',
            message: 'Fetched logs from interactions',
            data: mappedLogs
        };
    } catch (err: any) {
