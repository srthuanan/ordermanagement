...
export const getTestDriveSchedule = async (): Promise<ApiResult> => {
    try {
        ...
        const schedule = data.map(item => ({
            ...
            bienSo: item.bien_so, coSo: item.co_so, gplxHang: item.gplx_hang, cmndO: item.cmnd_o, cmndNoiCap: item.cmnd_noi_cap, cmndNgayCap: item.cmnd_ngay_cap
        }));
...
export const updateTestDriveCheckin = async (payload: { soPhieu: string; odoBefore?: string; imagesBefore?: any[]; odoAfter?: string; imagesAfter?: any[]; }): Promise<ApiResult> => {
    try {
        ...
        if (error) throw error;
        return { status: 'SUCCESS', message: 'Cập nhật thành công.', updatedRecord: { 
            soPhieu: ud.so_phieu, ngayThuXe: ud.ngay_thu_xe, loaiXe: ud.loai_xe, thoiGianKhoiHanh: ud.thoi_gian_khoi_hanh, thoiGianTroVe: ud.thoi_gian_tro_ve, loTrinh: ud.lo_trinh, tenKhachHang: ud.ten_khach_hang, dienThoai: ud.dien_thoai, email: ud.email, diaChi: ud.dia_chi, tuLai: String(ud.tu_lai), dacDiem: ud.dac_diem, gplxSo: ud.gplx_so, hieuLucGPLX: ud.hieu_luc_gplx, ngayCamKet: ud.ngay_cam_ket, tenTuVan: ud.ten_tu_van, odoBefore: ud.odo_before, imagesBefore: ud.images_before, odoAfter: ud.odo_after, imagesAfter: ud.images_after,
            bienSo: ud.bien_so, coSo: ud.co_so, gplxHang: ud.gplx_hang, cmndO: ud.cmnd_o, cmndNoiCap: ud.cmnd_noi_cap, cmndNgayCap: ud.cmnd_ngay_cap
        } };
