...
({ order, index, onViewDetails, onCancel, onRequestInvoice, onSupplement, onRequestVC, onConfirmVC, onRowClick, selectedOrder, viewMode, processingOrder }) => {
    ...
    return (
        ...
        >
            <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm text-center text-text-secondary font-medium sm:pl-6" data-label="#">{index + 1}</td>
            <td className="whitespace-nowrap px-3 py-4 text-sm" data-label="Khách hàng / SĐH">
            ...
            <td className="whitespace-nowrap px-3 py-4 text-sm" data-label="Thông Tin Xe">
            ...
            {viewMode === 'full' && (
                 <td className="whitespace-nowrap px-3 py-4 text-sm" data-label="Ngày Yêu Cầu">
            ...
            <td className="whitespace-nowrap px-3 py-4 text-sm text-text-primary truncate" data-label="Tư vấn" title={order["Tên tư vấn bán hàng"]}>{order["Tên tư vấn bán hàng"] || "N/A"}</td>
            {viewMode === 'full' && (
                 <>
                    <td className="whitespace-nowrap px-3 py-4 text-sm text-text-primary" data-label="Trạng thái"><StatusBadge status={statusText} /></td>
                    <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6" data-label="Hành động" onClick={(e) => e.stopPropagation()}>
                        {isProcessing ? (
