...
const MatchingCockpitView: React.FC<MatchingCockpitViewProps> = ({ pendingOrders, pairedOrders, stockData, onAction, filters }) => {
    ...
    return (
        <div className="flex h-full bg-surface-ground gap-4 p-4 animate-fade-in">
            {/* Left Column: Request Stream (40%) */}
            <div className="w-2/5 flex flex-col bg-surface-card rounded-xl shadow-md border border-border-primary overflow-hidden">
                {/* Tabs */}
                <div className="flex border-b border-border-primary">
                    <button
                        onClick={() => setActiveTab('pending')}
                        className={`flex-1 py-3 text-sm font-bold transition-colors ${activeTab === 'pending' && !filters?.keyword ? 'bg-accent-primary/10 text-accent-primary border-b-2 border-accent-primary' : 'text-text-secondary hover:bg-surface-hover'}`}
                    >
                    ...
                    <button
                        ...
                        className={`flex-1 py-3 text-sm font-bold transition-colors ${activeTab === 'paired' && !filters?.keyword ? 'bg-success/10 text-success border-b-2 border-success' : 'text-text-secondary hover:bg-surface-hover'}`}
                    >
                    ...
                    {filteredOrders.map(order => {
                        ...
                        return (
                            <div
                                ...
                                className={`p-3 rounded-lg border cursor-pointer transition-all relative overflow-hidden group ${isSelected ? 'border-accent-primary bg-accent-primary/5 shadow-sm' : 'border-border-secondary bg-white hover:border-accent-primary/50'}`}
                            >
                                {isPending ? (
                                    // PENDING ORDER LAYOUT
                                    <>
                                        <div className={`absolute left-0 top-0 bottom-0 w-1 ${stockStatus === 'success' ? 'bg-success' : stockStatus === 'warning' ? 'bg-warning' : 'bg-danger'}`}></div>
                                        <div className="pl-2">
                                            <div className="flex justify-between items-start mb-1">
                                                ...
                                                {exactMatchesCount > 0 ? (
                                                    <span className="animate-pulse bg-success text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm">
                                                        {exactMatchesCount} xe sẵn sàng
                                                    </span>
                                                ) : (
                                                    <span className="text-[10px] text-text-secondary">{moment(order['Thời gian nhập']).format('DD/MM')}</span>
                                                )}
                                            </div>
            ...
            <div className="w-3/5 flex flex-col bg-surface-card rounded-xl shadow-md border border-border-primary overflow-hidden">
                {selectedOrder ? (
                    <div className="flex-1 flex flex-col">
                        {/* Header */}
                        <div className="p-4 border-b border-border-primary bg-surface-ground/30">
                            <div className="flex justify-between items-start">
                                <div>
                                    <h2 className="text-xl font-bold text-text-primary mb-1">{selectedOrder['Tên khách hàng']}</h2>
                                    <div className="flex items-center gap-3 text-sm text-text-secondary">
                                        <span className="font-mono bg-white px-2 py-0.5 rounded border border-border-secondary">{selectedOrder['Số đơn hàng']}</span>
                        ...
                        {/* Content */}
                        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-surface-ground/10">
                            {/* Detailed Order Info Section */}
                            <div className="bg-white p-4 rounded-xl border border-border-secondary shadow-sm mb-4">
                                    ...
                                    {matchingSuggestions.length > 0 ? (
                                        <div className="grid grid-cols-1 gap-3">
                                            {matchingSuggestions.map(car => (
                                                <div key={car.VIN} className="bg-white p-4 rounded-xl border border-border-secondary shadow-sm hover:shadow-md transition-all flex items-center justify-between group">
