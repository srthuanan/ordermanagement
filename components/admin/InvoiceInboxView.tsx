...
const InvoiceInboxView: React.FC<InvoiceInboxViewProps> = ({ orders, onAction, showToast, onOpenFilePreview, onUpdateInvoiceDetails }) => {
    const [selectedFolder, setSelectedFolder] = useState<'all' | 'pending_approval' | 'approved' | 'request_supplement' | 'supplemented' | 'pending_signature' | 'completed'>('pending_approval');
                        ...
                        <div className="divide-y divide-border-secondary">
                            {filteredOrders.map(order => (
                                <div
                                    ...
                                    className={`p-3 cursor-pointer hover:bg-surface-hover transition-colors ${selectedOrderId === order['Số đơn hàng'] ? 'bg-accent-primary/5 border-l-4 border-accent-primary' : 'border-l-4 border-transparent'}`}
                                >
                                    <div className="flex justify-between items-start mb-1">
                                        <span className="font-semibold text-text-primary text-sm truncate flex-1 pr-2">{order['Tên khách hàng']}</span>
                                        <span className="text-xs text-text-secondary whitespace-nowrap">{moment(order['Thời gian nhập']).format('DD/MM')}</span>
                                    </div>
                                    <div className="text-xs text-text-secondary mb-1 truncate">{order['Dòng xe']} - {order['Phiên bản']}</div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-xs font-mono text-text-placeholder">{order['Số đơn hàng'].slice(-6)}</span>
