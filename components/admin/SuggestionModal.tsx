...
const SuggestionModal: React.FC<SuggestionModalProps> = ({ isOpen, onClose, onConfirm, order, suggestedCars, showToast }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-0 md:p-4" onClick={onClose}>
            <div className="bg-surface-card w-full md:max-w-2xl h-full md:h-auto md:max-h-[90vh] rounded-none md:rounded-2xl shadow-xl animate-fade-in-scale-up flex flex-col" onClick={e => e.stopPropagation()} style={bgStyle}>
                ...
                <header className="flex-shrink-0 flex items-start justify-between p-3">
                    <div className="flex items-start gap-2">
                        <div className="flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center bg-yellow-100">
                ...
                <main className="p-3 space-y-2 overflow-y-auto">
                    ...
                    <div>
                        ...
                        {suggestedCars.length > 0 ? (
                            <div className="max-h-64 overflow-y-auto border border-border-primary rounded-lg hidden-scrollbar">
                                <table className="min-w-full text-sm responsive-table">
                                    <thead className="bg-surface-hover sticky top-0">
                                        <tr>
                                            <th className="py-1 px-1.5 text-left w-10">Chọn</th>
                                            <th className="py-1 px-1.5 text-left">Số VIN</th>
                                            <th className="py-1 px-1.5 text-left">Nội Thất</th>
                                            <th className="py-1 px-1.5 text-left">Ngày Nhập Kho</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-border-primary bg-surface-card">
                                        {suggestedCars.map((car) => (
                                            <tr key={car.VIN} className="hover:bg-surface-hover">
                                                <td className="py-1.5 px-1.5" data-label="Chọn">
                                                ...
                                                <td className="py-1.5 px-1.5 font-mono text-text-primary" data-label="Số VIN">
                                                    <div
                                                ...
                                                <td className="py-1.5 px-1.5 text-text-secondary" data-label="Ngày Nhập Kho">{car['Thời gian nhập'] ? moment(car['Thời gian nhập']).format('DD/MM/YYYY') : 'N/A'}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                ...
                <footer className="flex-shrink-0 px-3 py-2 flex justify-end items-center gap-1.5 bg-surface-ground rounded-b-2xl border-t border-border-primary">
