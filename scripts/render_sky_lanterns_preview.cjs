...
async function run() {
  try {
    ...
    console.log('Rendering sky lanterns & houses overview...');
    await sharp(svgBuf)
      .extract({ left: 50, top: 20, width: 1820, height: 620 })
      .png()
      .toFile(`${scratchDir}/preview_sky_lanterns_and_houses.png`);

    console.log('Rendering Hoi Quan central sky lanterns close-up...');
    await sharp(svgBuf)
      .extract({ left: 620, top: 40, width: 680, height: 480 })
      .png()
      .toFile(`${scratchDir}/preview_thien_dang_center.png`);

    console.log('DONE ALL SKY LANTERN PREVIEWS!');
  } catch (e) {
