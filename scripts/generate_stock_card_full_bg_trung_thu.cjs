  ...
  return `
    <g transform="translate(${x.toFixed(1)}, ${y.toFixed(1)}) scale(${scale})" opacity="${opacity.toFixed(2)}">
      <path d="M 0,20 Q 15,10 30,16 Q 42,0 60,6 Q 78,-2 95,12 Q 110,6 125,18 Q 138,32 120,38 Q 100,42 65,40 Q 30,42 12,34 Q -4,30 0,20 Z" 
            fill="url(#cloudGrad)" />
      <path d="M 22,22 Q 36,14 54,20 Q 72,12 92,20" fill="none" stroke="#fde047" stroke-width="0.8" opacity="0.45"/>
    </g>
  `;
}

async function main() {
  console.log('--- BẮT ĐẦU TẠO ẢNH NỀN ĐỘNG TRUNG THU CHO TOÀN BỘ THẺ XE (FULL STOCK CARD) ---');
  ...
  for (let f = 0; f < TOTAL_FRAMES; f++) {
    ...
    const moonAuraR = 52 + 5 * Math.sin(phase);
    const moonAuraOpacity = 0.55 + 0.18 * Math.sin(phase);

    // Mây trôi ngang qua trăng và giữa thẻ
    const cloud1X = 160 + Math.sin(phase) * 14;
    const cloud2X = 30 - Math.cos(phase) * 12;
    const cloud3X = 120 + Math.sin(phase + 1.5) * 10;

    // Ngôi sao lấp lánh rải rác trên toàn thẻ
    ...
    // Đom đóm bay lơ lửng khắp thẻ từ trên xuống dưới
    const firefly1X = 75 + Math.sin(phase * 2) * 10;
    const firefly1Y = 220 - Math.cos(phase * 2) * 14;
    const firefly2X = 240 + Math.cos(phase * 2) * 12;
    const firefly2Y = 320 + Math.sin(phase * 2) * 12;
    const firefly3X = 110 - Math.sin(phase * 2) * 8;
    const firefly3Y = 410 + Math.cos(phase * 2) * 10;

    const svgContent = `
      <svg width="${WIDTH}" height="${HEIGHT}" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <!-- Bầu trời đêm Trung Thu hoàng gia sâu thẳm bao trùm toàn thẻ -->
          <linearGradient id="fullNightSky" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stop-color="#070c18"/>
            <stop offset="25%" stop-color="#0c1428"/>
            <stop offset="60%" stop-color="#111a34"/>
            <stop offset="85%" stop-color="#0a1020"/>
            <stop offset="100%" stop-color="#060913"/>
          </linearGradient>

          <!-- Mặt sàn phản chiếu ánh trăng nơi xe đậu -->
          <linearGradient id="floorAura" x1="0%" y1="0%" x2="0%" y2="100%">
        ...
        <!-- Thân nền toàn thẻ bo góc -->
        <rect x="0" y="0" width="${WIDTH}" height="${HEIGHT}" rx="16" fill="url(#fullNightSky)"/>

        <!-- Mặt sàn ánh trăng nhẹ nơi xe đậu (khoảng Y = 80..120) -->
        <ellipse cx="160" cy="115" rx="140" ry="20" fill="url(#floorAura)"/>

        <!-- Bụi sao đêm lấp lánh rải rác trên thẻ -->
        <circle cx="45" cy="45" r="1.1" fill="#ffffff" opacity="0.75"/>
        <circle cx="120" cy="22" r="1.3" fill="#fef08a" opacity="0.8"/>
        <circle cx="285" cy="30" r="1.0" fill="#ffffff" opacity="0.6"/>
        <circle cx="35" cy="165" r="1.1" fill="#fef08a" opacity="0.7"/>
        <circle cx="290" cy="210" r="1.2" fill="#ffffff" opacity="0.65"/>
        <circle cx="40" cy="360" r="1.0" fill="#fef08a" opacity="0.6"/>
        <circle cx="270" cy="420" r="1.2" fill="#ffffff" opacity="0.7"/>

        ${renderSparkle(135, 30, 3.5, s1)}
        ${renderSparkle(280, 55, 3.2, s2)}
        ${renderSparkle(50, 240, 2.8, s3)}
        ${renderSparkle(275, 340, 3.0, s4)}

        <!-- Quầng hào quang Vầng Trăng Rằm (ở góc trên phải) -->
        <circle cx="245" cy="48" r="${moonAuraR.toFixed(1)}" fill="url(#moonGlow)"/>

        <!-- Vầng Trăng Rằm 3D Hoàng Kim -->
        <circle cx="245" cy="48" r="28" fill="url(#moonSurface)"/>
        <!-- Vân bóng nguyệt chìm trên trăng -->
        <path d="M 238,40 Q 244,34 250,38 Q 255,46 248,54 Q 240,50 238,40 Z" fill="#d97706" opacity="0.16"/>
        <circle cx="256" cy="44" r="4.5" fill="#b45309" opacity="0.12"/>

        <!-- Các cụm vân mây hoàng kim trôi bồng bềnh -->
        ${renderCloud(cloud1X, 46, 0.65, 0.40)}
        ${renderCloud(cloud2X, 22, 0.50, 0.30)}
        ${renderCloud(cloud3X, 420, 0.55, 0.25)}

        <!-- ĐÈN ÔNG SAO MINI ĐU ĐƯA Ở GÓC TRÁI TRÊN CỦA THẺ -->
        <g transform="translate(32, 0)">
          ...
          <g transform="translate(0, 32) rotate(${starLanternSway.toFixed(2)})">
            ...
            <circle cx="0" cy="0" r="14" fill="#f59e0b" opacity="0.35"/>
            ...
            <line x1="0" y1="11" x2="0" y2="28" stroke="#ef4444" stroke-width="1.4" stroke-linecap="round"/>
            <line x1="-2.5" y1="10" x2="-4" y2="23" stroke="#f59e0b" stroke-width="0.9" stroke-linecap="round"/>
            <line x1="2.5" y1="10" x2="4" y2="23" stroke="#38bdf8" stroke-width="0.9" stroke-linecap="round"/>
            <circle cx="0" cy="28" r="1.3" fill="#fbbf24"/>
          </g>
        </g>

        <!-- Đom đóm bay lơ lửng khắp thân thẻ -->
        ...
        <g transform="translate(${firefly3X.toFixed(1)}, ${firefly3Y.toFixed(1)})">
          <circle cx="0" cy="0" r="3.2" fill="#fde047" opacity="0.35"/>
  ...
  console.log('Đang đóng gói file WebP động Full Stock Card (Muxing ANMF chunks)...');
  const animatedWebpBuffer = muxAnimatedWebP(frames, DELAY, 0);

  const outPicturesPath = path.join(__dirname, '../pictures/stock_card_full_bg_trung_thu.webp');
  const outPublicPath = path.join(__dirname, '../public/assets/stock_card_full_bg_trung_thu.webp');
  ...
  console.log(`HOÀN TẤT! File ảnh nền động Full Stock Card: ${sizeKb} KB`);
  console.log(`Đã lưu tại:\n- ${outPicturesPath}\n- ${outPublicPath}`);
}

main().catch(err => {
  console.error('Lỗi khi tạo ảnh nền Full Stock Card:', err);
  process.exit(1);
});
