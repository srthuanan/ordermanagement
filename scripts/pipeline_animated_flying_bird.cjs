...
const HEIGHT = 480;
const FRAMES = 32;
const DELAY = 75; // 75ms per frame (~13.3 FPS silky wing flapping)
...
// -----------------------------------------------------------------------------
// 🕊️ CHU TRÌNH VỖ CÁNH ĐỘNG HỌC KHÔNG GIAN 3D CỦA CHIM (AERODYNAMIC FLAP CYCLE)
// - Khối thân chim bồ câu trắng tinh khôi giữ vững hình dạng và đồng bộ 100%
// - Đôi cánh sải rộng uốn lượn hình sin đón gió (Upstroke -> Downstroke)
// - Thân chim nhấp nhô nhịp nhàng theo lực đẩy khí động học (Vertical Lift Drift)
// - Đuôi xòe lái gió uyển chuyển mềm mại
// -----------------------------------------------------------------------------
function renderFlyingBirdFrame(frameIndex) {
  const rad = (frameIndex / FRAMES) * Math.PI * 2;

  // 1. CHU KỲ VỖ CÁNH UỐN LƯỢN HÌNH SIN (WING FLAP $\pm 42^\circ$)
  const wingAngleDeg = Math.sin(rad) * 42; // -42° (đập xuống) -> +42° (giương cao)
  const wingRad = (wingAngleDeg * Math.PI) / 180;

  // 2. LỰC NÂNG KHÍ ĐỘNG HỌC LÀM THÂN CHIM NHẤP NHÔ (BODY LIFT DRIFT)
  const bodyLiftY = Math.sin(rad + 0.3) * 14;
  const bodyCenterY = 240 + bodyLiftY;
  const bodyCenterX = 280;

  // 3. ĐỘ TRỄ ĐÀN HỒI CỦA ĐẦU CÁNH VÀ ĐUÔI (FEATHER TIP WAVE LAG)
  const tipLag = Math.sin(rad - 0.5) * 18;
  const tailLag = Math.sin(rad - 0.8) * 12;

  // Tọa độ khớp cánh trái & phải
  ...
  const wingSpanY = -140 * Math.sin(wingRad);
  ...
  return `
    <svg width="${WIDTH}" height="${HEIGHT}" viewBox="0 0 ${WIDTH} ${HEIGHT}" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <!-- Lông vũ trắng tuyết pha ánh bạc & lam ngọc -->
      ...
      <g filter="url(#birdDepth3D)">
        <!-- 2. ĐUÔI CHIM LÁI GIÓ (TAIL FEATHERS) -->
        ...
        <!-- 3. CÁNH TRÁI (LEFT WING) VỖ THEO KHÍ ĐỘNG HỌC -->
        <g filter="url(#featherSoft)">
          ...
          <!-- Từng nan lông vũ cánh trái xếp lớp -->
        ...
        <!-- 4. CÁNH PHẢI (RIGHT WING) VỖ ĐỐI XỨNG HOÀN HẢO -->
        <g filter="url(#featherSoft)">
          ...
          <!-- Từng nan lông vũ cánh phải xếp lớp -->
        ...
        <g transform="translate(${bodyCenterX}, ${bodyCenterY})">
          ...
          <!-- Đôi mắt đen láy lấp lánh đốm sáng -->
          <!-- Mắt trái -->
...
async function main() {
  console.log('🕊️ Rendering 100% Flawless Aerodynamic Flying Bird Animation (Pure Transparent Background)...');

  const frameBuffers = [];

  for (let f = 0; f < FRAMES; f++) {
    const svg = renderFlyingBirdFrame(f);
  ...
  const filesToSave = [
    ...
    path.join(__dirname, '../pictures/flying_bird_animated_transparent.webp')
  ];
  ...
  console.log(`🎉 100% Flawless Flying Bird Animation rendered successfully:`);
